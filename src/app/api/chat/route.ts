...
export async function POST(request: NextRequest) {
  return withRequestContext(request, async () => {
    ...
    try {
      ...
      if (!rl.allowed) {
        logger.warn('[CHAT_POST] Rate limit exceeded', { clientIp });
        return error(new RateLimitError('Too many requests. Please wait a moment.', rl.retryAfterMs), timings.meta());
      }
      ...
      if (llmValidation.risk >= 0.7) {
        logger.warn('[CHAT_SECURITY] Prompt injection blocked', {
          sessionId: sid,
      ...
      if (llmValidation.risk >= 0.3) {
        logger.warn('[CHAT_SECURITY] Prompt injection risk detected — using sanitized input', {
          sessionId: sid,
          risk: llmValidation.risk,
          warnings: llmValidation.warnings,
        });
